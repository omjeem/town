...
function LeaderboardRow({ rank, row }: { rank: number; row: Row }) {
  ...
  return (
    <li className="flex items-center gap-2 border-b border-paper/10 px-3 py-2 last:border-b-0 hover:bg-white/5">
      <span
      ...
      >
        {row.itemCount} · {row.tagCount}
      </span>
      <span className="w-8 shrink-0 text-right font-mono text-xs font-bold text-paper">
        {row.score}
      </span>
    </li>
  );
